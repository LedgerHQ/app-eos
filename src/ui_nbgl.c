...
static void app_settings() {
    nbgl_useCaseSettings(APPNAME, 0, 2, false, ui_idle, nav_callback, controls_callback);
}
...
void ui_display_single_action_sign_flow(void) {
    if (txProcessingCtx.currentActionNumer > 1) {
        snprintf(review_title, sizeof(review_title), "Review action #%d", txProcessingCtx.currentActionIndex);
    } else {
